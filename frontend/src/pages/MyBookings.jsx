...
export default function MyBookings() {
  ...
  return (
    <section className="relative overflow-hidden rounded-[2rem] border border-slate-200 bg-[linear-gradient(180deg,#f8fafc_0%,#eef2ff_100%)] text-slate-900 shadow-[0_30px_70px_-42px_rgba(15,23,42,0.45)]">
      ...
      <div className="relative space-y-6 p-5 md:p-7">
        <header className="rounded-[2rem] border border-slate-200/80 bg-white/90 p-5 shadow-[0_18px_40px_-30px_rgba(15,23,42,0.35)] backdrop-blur md:p-6">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div className="max-w-3xl">
              <p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-slate-500">Bookings Workspace</p>
              <h1 className="mt-2 text-4xl font-semibold tracking-tight text-slate-950 md:text-5xl">
                Modern booking operations, one place.
              </h1>
              <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-600">
                Track requests, review your timeline, and launch a new booking from a dashboard designed to feel clean, fast, and professional.
              </p>
            </div>
            ...
            <button
              ...
              className="rounded-2xl bg-slate-950 px-5 py-3 text-sm font-semibold text-white shadow-[0_12px_26px_-18px_rgba(15,23,42,0.65)] transition hover:-translate-y-0.5 hover:bg-slate-800"
            >
              + New Request
            </button>
          </div>
        ...
        <section className="grid gap-6 xl:grid-cols-[320px_minmax(0,1fr)]">
          <aside className="space-y-6">
            <div className="relative overflow-hidden rounded-[2rem] border border-slate-200 shadow-[0_18px_40px_-28px_rgba(15,23,42,0.6)]">
              <div
                className="absolute inset-0 bg-cover bg-center"
                style={{
                  backgroundImage: "linear-gradient(140deg, rgba(15,23,42,0.88), rgba(14,116,144,0.65)), url('/study2.jpg')",
                }}
              />
              <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,rgba(125,211,252,0.18),transparent_38%)]" />
            ...
            <div className="rounded-[2rem] border border-slate-200 bg-white/92 p-5 shadow-[0_18px_42px_-32px_rgba(15,23,42,0.45)] backdrop-blur md:p-6">
              <div className="flex items-center justify-between gap-3">
                <div>
                  <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-slate-500">Focus</p>
                  <h3 className="mt-1 text-lg font-semibold tracking-tight text-slate-950">Quick Action</h3>
                </div>
              </div>
              <p className="mt-3 text-sm leading-6 text-slate-600">
                Start a new booking from the dashboard without leaving the page.
              </p>
              <button
                type="button"
                onClick={() => setIsModalOpen(true)}
                className="mt-5 w-full rounded-2xl bg-slate-950 px-4 py-3 text-sm font-semibold text-white shadow-[0_12px_26px_-18px_rgba(15,23,42,0.7)] transition hover:-translate-y-0.5 hover:bg-slate-800"
              >
                + New Request
              </button>
            </div>
            ...
            <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-1">
              <div
                className="group rounded-[1.5rem] border border-slate-200 bg-white/92 p-5 shadow-[0_16px_34px_-28px_rgba(15,23,42,0.45)] backdrop-blur transition duration-300 hover:-translate-y-0.5 hover:shadow-[0_20px_38px_-24px_rgba(15,23,42,0.5)]"
                style={{ animation: 'insightRise 600ms ease-out 90ms both' }}
              >
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-slate-400">Peak Hours</p>
                    <h4 className="mt-1 text-base font-semibold text-slate-900">Most bookings</h4>
                  </div>
                  <div className="rounded-full bg-slate-100 p-2 transition duration-300 group-hover:scale-105">
                    <span className="block h-3 w-3 rounded-full border-2 border-slate-500 border-t-transparent" />
                  </div>
                </div>
                ...
                <p className="mt-1 text-xs text-slate-500">
                  {bookingInsights.peakHour === null
                    ...
                    : `${bookingInsights.peakCount} booking${bookingInsights.peakCount === 1 ? '' : 's'} in that window`}
                </p>
              </div>
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-slate-400">Avg Approval Time</p>
                    <h4 className="mt-1 text-base font-semibold text-slate-900">Approval speed</h4>
                  </div>
                  <div className="rounded-full bg-slate-100 p-2 transition duration-300 group-hover:scale-105">
                    <span className="block h-3 w-3 rounded-full border-2 border-slate-500 border-b-transparent" />
                  </div>
                </div>
                <p className="mt-4 text-lg font-semibold text-slate-950">~{bookingInsights.avgApprovalHours.toFixed(1)} hours</p>
                <p className="mt-1 text-xs text-slate-500">Based on approved bookings only</p>
              </div>
            </div>
          </aside>

          <main className="space-y-6">
            <article className="rounded-[2rem] border border-slate-200 bg-white/92 p-5 shadow-[0_18px_44px_-32px_rgba(15,23,42,0.45)] backdrop-blur md:p-6">
              <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
                <div>
                  <p className="text-[10px] font-semibold uppercase tracking-[0.22em] text-slate-500">Timeline</p>
                  <h2 className="mt-1 text-2xl font-semibold tracking-tight text-slate-950">Next Up Bookings</h2>
                </div>
              ...
              {nextUpBookings.length === 0 ? (
                ...
                <div className="space-y-4">
                  {nextUpBookings.map((booking, index) => (
                    <article
                      key={`next-${booking.id}`}
                      className="group relative overflow-hidden rounded-2xl border border-slate-200 bg-[linear-gradient(180deg,#ffffff_0%,#f8fafc_100%)] p-4 transition duration-200 hover:-translate-y-0.5 hover:shadow-[0_18px_32px_-20px_rgba(15,23,42,0.45)]"
                    >
                      ...
                      <div className="mt-4 grid gap-2 sm:grid-cols-3 pl-2">
            ...
            <article className="rounded-[2rem] border border-slate-200 bg-white/92 p-5 shadow-[0_18px_42px_-32px_rgba(15,23,42,0.45)] backdrop-blur md:p-6">
